...
export default function KPIDashboardPage() {
  ...
  return (
    <div className="min-h-screen space-y-6 pb-24 sm:pb-12 text-slate-900 dark:text-slate-100">
      ...
      <div className="relative overflow-hidden rounded-2xl sm:rounded-3xl bg-gradient-to-r from-orange-50/70 via-white to-amber-50/60 dark:from-slate-900 dark:via-slate-900 dark:to-slate-850 border border-orange-200/80 dark:border-slate-800 p-4 sm:p-6 lg:p-8 shadow-sm space-y-4 sm:space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
          <div className="flex items-start sm:items-center gap-3.5 sm:gap-4">
            <div className="w-11 h-11 sm:w-14 sm:h-14 bg-orange-500 text-white rounded-2xl shrink-0 shadow-md flex items-center justify-center">
              <BarChart3 className="w-5 h-5 sm:w-7 sm:h-7 stroke-[2]" />
            </div>
            <div className="space-y-1 min-w-0 flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] sm:text-xs font-bold bg-orange-100 dark:bg-orange-950/60 text-orange-700 dark:text-orange-400 border border-orange-200 dark:border-orange-800/80 shrink-0">
        ...
        {/* Header Actions */}
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-3 border-t border-slate-200/60 dark:border-slate-800 relative z-10">
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <select
              value={timeRange}
              onChange={e => setTimeRange(e.target.value)}
              className="flex-1 sm:flex-initial px-3.5 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl text-xs font-bold text-slate-800 dark:text-slate-200 outline-none cursor-pointer shadow-xs focus:border-orange-500 transition-all min-w-0"
            >
            ...
            <button
              ...
              disabled={refreshing}
              className="p-2.5 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 rounded-2xl text-slate-600 dark:text-slate-300 transition-colors cursor-pointer shadow-xs shrink-0"
          ...
          <button
            ...
            className="w-full sm:w-auto px-5 py-2.5 bg-gradient-to-r from-orange-500 to-amber-600 hover:from-orange-600 hover:to-amber-700 active:scale-95 text-white font-extrabold text-xs rounded-2xl transition-all cursor-pointer shadow-md shadow-orange-500/20 flex items-center justify-center gap-2"
          >
      ...
      {/* ── Summary Velocity Stats (Mobile-Optimized Clean Cards) ── */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {[
          { label: 'Velocity Score', value: `${calculateOverallPerformance()}%`, sub: 'Avg Target', icon: Gauge, color: 'text-orange-600 dark:text-orange-400', bg: 'bg-orange-50 dark:bg-orange-950/60' },
          { label: 'Trending Up', value: kpis.filter(k => k.changeType === 'increase').length, sub: 'Accelerating', icon: TrendingUp, color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-950/60' },
          { label: 'On Target', value: kpis.filter(k => k.progress !== undefined && k.progress >= 85).length, sub: `of ${kpis.length} Goals`, icon: Target, color: 'text-purple-600 dark:text-purple-400', bg: 'bg-purple-50 dark:bg-purple-950/60' },
          ...
          <div key={stat.label} className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl sm:rounded-3xl p-3.5 sm:p-5 shadow-xs hover:border-orange-300 dark:hover:border-orange-900/50 transition-all flex flex-col justify-between min-w-0">
            <div className="flex items-center justify-between gap-2 mb-2">
              <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-xl ${stat.bg} flex items-center justify-center shrink-0 shadow-2xs`}>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl border border-slate-200/80 dark:border-slate-800 p-3 sm:p-4 shadow-xs flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 sm:gap-4">
        
        {/* Category Chips Bar — Smooth Horizontal Scroll on Mobile */}
        <div className="flex items-center gap-2 overflow-x-auto pb-1 sm:pb-0 min-w-0 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
          {metricCategories.map(cat => {
            ...
            return (
              <div
                ...
                className={`px-3.5 py-2 rounded-2xl text-xs font-bold whitespace-nowrap transition-all cursor-pointer flex items-center gap-2 shrink-0 ${
                  isActive
        ...
        <div className="flex items-center justify-between sm:justify-start gap-1 bg-slate-100 dark:bg-slate-800/80 p-1 sm:p-1.5 rounded-xl sm:rounded-2xl shrink-0 overflow-x-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
          <button
            onClick={() => setViewMode('grid')}
            className={`flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all cursor-pointer ${
      ...
      {/* ── Add Target KPI Modal ── */}
      {showAddKPI && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-3 pb-24 sm:pb-4 md:p-6 overflow-hidden">
          <div className="absolute inset-0 bg-slate-950/70 backdrop-blur-xs" onClick={() => setShowAddKPI(false)} />
          <div className="relative w-full max-w-lg bg-white dark:bg-slate-950 border border-slate-200/80 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[calc(100dvh-7.5rem)] sm:max-h-[85vh] transform animate-in slide-in-from-bottom-4 duration-300">
            ...
            <div className="shrink-0 px-5 sm:px-6 py-3.5 sm:py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/80 dark:bg-slate-900/80">
              <div className="flex items-center gap-2.5">
                ...
                <h3 className="text-sm sm:text-base font-extrabold text-slate-900 dark:text-white tracking-tight">Add Target KPI Stream</h3>
              </div>
              <button onClick={() => setShowAddKPI(false)} className="p-1.5 text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-all cursor-pointer">
                <X className="w-5 h-5" />
              </button>
            </div>

            {/* Modal Body - Scrollable */}
            <div className="p-4 sm:p-6 space-y-4 flex-1 min-h-0 overflow-y-auto overscroll-contain custom-scrollbar">
              <div>
            ...
            {/* Modal Footer - Guaranteed Sticky & Visible */}
      ...
      {showEditModal && editingKPI && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-3 pb-24 sm:pb-4 md:p-6 overflow-hidden">
          <div className="absolute inset-0 bg-slate-950/70 backdrop-blur-xs" onClick={() => { setShowEditModal(false); setEditingKPI(null); }} />
          <div className="relative w-full max-w-lg bg-white dark:bg-slate-950 border border-slate-200/80 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[calc(100dvh-7.5rem)] sm:max-h-[85vh]">
            <div className="shrink-0 px-5 sm:px-6 py-3.5 sm:py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/80 dark:bg-slate-900/80">
              <h3 className="text-sm sm:text-base font-extrabold text-slate-900 dark:text-white">Edit Target KPI</h3>
              <button onClick={() => { setShowEditModal(false); setEditingKPI(null); }} className="p-1.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 rounded-xl">
                <X className="w-5 h-5" />
              </button>
            </div>
      ...
      {showDeleteModal && deletingKPI && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-3 pb-24 sm:pb-4 overflow-hidden">
          <div className="absolute inset-0 bg-slate-950/70 backdrop-blur-xs" onClick={() => { setShowDeleteModal(false); setDeletingKPI(null); }} />
          <div className="relative w-full max-w-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col">
            <div className="p-6 space-y-3">
              <div className="w-12 h-12 rounded-2xl bg-rose-50 dark:bg-rose-950/60 flex items-center justify-center text-rose-600 shadow-xs">
                <Trash2 className="w-6 h-6" />
              </div>
              <h3 className="text-base font-extrabold text-slate-900 dark:text-white">Delete &ldquo;{deletingKPI.name}&rdquo;?</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">This target metric will be permanently removed. This action cannot be undone.</p>
            </div>
            <div className="shrink-0 p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 flex items-center justify-end gap-3">
              <button
                type="button"
                onClick={() => { setShowDeleteModal(false); setDeletingKPI(null); }}
                className="flex-1 py-2.5 px-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl text-xs font-bold text-slate-700 dark:text-slate-300"
              >
                Cancel
              </button>
              <button
                ...
                className="flex-1 py-2.5 px-4 bg-rose-600 hover:bg-rose-700 disabled:opacity-40 text-white font-bold text-xs rounded-2xl shadow-xs flex items-center justify-center gap-2"
              >
      ...
      {showAnalysisModal && analysisKPI && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-3 pb-24 sm:pb-4 md:p-6 overflow-hidden">
          <div className="absolute inset-0 bg-slate-950/70 backdrop-blur-xs" onClick={() => setShowAnalysisModal(false)} />
          <div className="relative w-full max-w-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[calc(100dvh-7.5rem)] sm:max-h-[85vh]">
            <div className="shrink-0 px-5 sm:px-6 py-3.5 sm:py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/80 dark:bg-slate-900/80">
              <div className="flex items-center gap-2">
                <Sparkles className="w-4 h-4 text-purple-600" />
                <h3 className="text-sm sm:text-base font-extrabold text-slate-900 dark:text-white">Okleevo AI Metric Intelligence</h3>
              </div>
              <button onClick={() => setShowAnalysisModal(false)} className="p-1.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 rounded-xl">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-4 sm:p-6 space-y-4 flex-1 min-h-0 overflow-y-auto custom-scrollbar">
              <div className="p-4 bg-slate-50 dark:bg-slate-800/60 rounded-2xl border border-slate-200/60 dark:border-slate-700/60 flex items-center justify-between">
                <div>
                  <h4 className="text-sm font-extrabold text-slate-900 dark:text-white">{analysisKPI.name}</h4>
                  <p className="text-[10px] text-slate-400 uppercase font-mono font-bold">{analysisKPI.category} Target</p>
                </div>
                <span className="text-lg font-black font-mono text-indigo-600 dark:text-indigo-400">{analysisKPI.value}</span>
              </div>

              <div className="p-4 bg-purple-50 dark:bg-purple-950/60 rounded-2xl border border-purple-100 dark:border-purple-900/40 space-y-2 min-h-[84px]">
            ...
            <div className="shrink-0 p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 flex items-center justify-end">
              <button
                type="button"
                onClick={() => setShowAnalysisModal(false)}
                className="w-full sm:w-auto px-6 py-2.5 bg-slate-900 dark:bg-slate-800 hover:bg-slate-800 text-white font-extrabold text-xs rounded-2xl shadow-xs cursor-pointer"
              >
